...
namespace Avalonia.Extensions.Hosting
{
    ...
    /// </summary>
    public sealed class
        AvaloniaApplicationBuilder<TApplication, TWindow> : IAvaloniaApplicationBuilder<TApplication, TWindow>
        where TApplication : Application, new()
        where TWindow : Window
    {
        public HostBuilder HostBuilder { get; } = new();
        private readonly AvaloniaApplicationServiceCollection _services = new();
        ...
        private AvaloniaApplication<TApplication, TWindow>? _builtApplication;

        internal AvaloniaApplicationBuilder(AvaloniaApplicationOptions options)
        {
            Services = _services;
            _options = options;
            ...
            _appBuilder = AppBuilder.Configure<TApplication>();
            Host = new ConfigureHostBuilder(hostContext, Configuration, Services, _appBuilder);
        ...
        public AvaloniaApplication<TApplication, TWindow> Build()
        {
            ...
            {
                if (_appBuilder.Instance is null)
                {
                    _appBuilder.SetupWithClassicDesktopLifetime(_options.Args ?? []);
                }

                return (TApplication)_appBuilder.Instance!;
            });
            Services.AddSingleton<TApplication>(_ =>
            {
                if (_appBuilder.Instance is null)
                {
                    _appBuilder.SetupWithClassicDesktopLifetime(_options.Args ?? []);
            ...
            // Copy the services that were added via AvaloniaApplicationBuilder.Services into the final IServiceCollection
            ...
            _builtApplication = new AvaloniaApplication<TApplication, TWindow>(HostBuilder.Build());
